...
def test_platform_limits_by_plan():
    assert get_platforms_allowed("trial", "trialing") == 5
    assert get_platforms_allowed("repurposer", "active") == 3
...
def test_plan_from_price_id(monkeypatch):
    monkeypatch.setattr(settings, "stripe_repurposer_price_id", "price_repurposer")
    ...
    monkeypatch.setattr(settings, "stripe_agency_price_id", "price_agency")

    assert plan_from_price_id("price_repurposer") == "repurposer"
    assert plan_from_price_id("price_creator") == "creator"
    assert plan_from_price_id("price_pro") == "pro"
    assert plan_from_price_id("price_agency") == "agency"
