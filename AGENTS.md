# AGENTS

This file is stable, human-owned guidance for agents.

- Inspect the repo before making changes.
- Keep diffs minimal.
- Do not refactor unrelated code.
- Reuse existing patterns before introducing new abstractions.
- Summarize files changed and validation performed.
- Read `HANDOFF_CONTEXT.md` before starting work and update it before handing work to another agent.
- Do not edit `AGENTS.md` automatically.
- If you discover a recurring rule, suggest it to the user instead of changing `AGENTS.md`. 

 you can update `WORKLOG.md` with important findings, and information relating to the project.





 # special request

 everytime i apporve of a plan you can vps deploy after your done
